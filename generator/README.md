...
5. **`deriver`** — reproduces the alerting math exactly (1 min rate → SUM over shards → ratio → 1 h smoothing → static alert), plus an illustrative trajectory-aware rule. A fully missing minute is not accepted as a zero-rate boundary: that minute and the first observed minute after it have no valid rate, then calculation recovers once two adjacent trustworthy boundaries exist. A reset still clears the baseline even when its own scrape is missing, so the first post-reset value is never compared with the prior counter lifetime.
...
4. **Monotone counters between resets.** Observed counters are non-decreasing except at flagged `reset_flag` steps (task restarts). A reset starts a new counter lifetime even when the reset scrape itself is missing.
5. **Exact reset- and gap-aware derived signals.** Derived `input_rate` and `output_rate` are ≥ 0 and exactly match an independent re-derivation from the raw counters. A fully missing shard-minute and that shard's first following delta are invalid because they do not have adjacent trustworthy boundaries; the following adjacent observed minute recovers normally. Invalid shards are omitted from the consumer sum, and the consumer rate is `NaN` only when every shard is invalid. Forward filling starts a new counter lifetime at zero when a reset scrape is missing, so `[100, NaN(reset), 5]` yields a valid post-reset increment of `5`, not a false decrease. Autogram's materialized shard-family sum and streaming `sum_delta` relation use the same state-machine contract.
6. **Exact generated derived grid.** The generator emits exactly `n_consumers` distinct, non-missing typed consumer identities and `n_consumers × n_minutes` derived rows. Every derived frame must include `archetype`, and every emitted value must equal its record's `consumer.archetype` with typed, missing-safe comparison. Within every consumer record, `minute_index` is exactly `0..n_minutes-1` in row order, `timestamp == start_timestamp + minute_index × rate_window_seconds`, and every row carries that record's consumer identity. Across all records, each typed `(consumer_id, minute_index)` identity occurs exactly once; nested tuple identities are valid when every component is non-missing. Malformed row counts fail validation rather than raising. This is a generator-output contract; the Autogram loader intentionally still accepts valid slices and gaps.
7. **Exact generated raw output.** When raw output is emitted, `check_all(..., raw)` requires the complete configured schema: identities, both counters, `missing_flag`, `reset_flag`, and (when enabled) both hidden-state columns. It also requires exactly the consumers and per-consumer shards declared by the generated records. Typed shard IDs must be non-missing and unique within each consumer (so `True`, `1`, `"1"`, and `1.0` remain distinct), while valid nested tuple identities are preserved. Rows must appear in consumer/shard/step order with `timestamp == start_timestamp + step × raw_scrape_seconds`, every row must carry its owning consumer and shard identity, and each global `(timestamp, consumer_id, shard_id)` identity must occur exactly once. Every counter, flag, and hidden value must exactly equal its generator record, with floating-point `NaN` matching only floating-point `NaN`. Missing columns, changed payloads, extra/reordered/malformed rows, or malformed identities produce a hard invariant failure rather than an exception.
...
| alerting (Q-D) | `alert_threshold`, `good_data_threshold`, `alert_duration_minutes` | 0.99, 0.98, 10 | both thresholds configurable; 0.99≠0.98; duration is a positive whole number of minutes |
