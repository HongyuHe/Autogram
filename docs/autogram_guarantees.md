...
- **(S2) Finite arithmetic.** `Fin(r)`: the rule's own expression must not exceed `float64` on any row it is offered. An overflow is a property of the *candidate*, not of the data — the operands were finite and the expression blew up on them — so such a row is neither dropped as missing nor counted as evidence; a candidate that overflows on more than `max_overflow_fraction` of its attempted rows (default `0`, i.e. any row at all) is refused with a reason naming the blow-up (`dsl/evaluate.py::ground` records `Grounded.overflow_points`; `discovery/evaluate.py::_overflow_rejection` enforces it). Without this a rule could be graded on whatever rows survived a finite mask while its reported support still described the full population.
